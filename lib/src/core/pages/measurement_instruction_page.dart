            ...
            SafeArea(
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final mascotH = (maxH * 0.7).clamp(220.0, 520.0).toDouble();
                  final mascotW = mascotH * Mascot.aspect;

                  final contentW =
                      (maxW * 0.94).clamp(360.0, 1100.0).toDouble();
                  final mascotColumnW = mascotW.clamp(0.0, contentW * 0.42);
                  final leftColumnW = contentW - mascotColumnW;

                  final panelH = (maxH * 0.42).clamp(180.0, 360.0).toDouble();

                  final buttonW =
                      (leftColumnW * 0.78).clamp(220.0, 460.0).toDouble();
                  final buttonH = (buttonW / 3.45).clamp(56.0, 96.0).toDouble();
